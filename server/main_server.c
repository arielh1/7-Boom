...
void MainServer(int port) {
	...
	for (Ind = 0; Ind < NUM_OF_WORKER_THREADS; Ind++) {
		...
		array_socket[Ind + 1] = INVALID_SOCKET;
	}
	Ind = 0;
	game_start();
...
int game_view(thread_service_arg* thread_argv) {
	...
	if (strlen(player_move[(thread_argv->player_index) % 2])) {
		...
		decode_message(message_to_client, &message, SENT_SERVER);
	...
	while (game_on) {
		...
		if (game_on == 1) {
			...
			if ((number_of_player != 2)&& (flag_opponent_quit==0)){
				...
				decode_message("GAME_ENDED", &message, SENT_SERVER);
				free_message(&message);
				if (write_to_file(thread_argv->file_name, message.log_file_format) != SUCCESS_CODE) {
					printf(WRITE_TO_FILE_ERROR_MESSAGE);
					return ERROR_CODE;
...
int server_state(thread_service_arg* thread_argv) {
	...
	while (1) {
		...
		case 2:
			...
			decode_message("GAME_STARTED", &message, SENT_SERVER);
			free_message(&message);
			if (write_to_file(thread_argv->file_name, message.log_file_format) != SUCCESS_CODE) {
				printf(WRITE_TO_FILE_ERROR_MESSAGE);
				return ERROR_CODE;
